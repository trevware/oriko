...
- **The exceptions are frontmatter, never the body**, and only ever through `app.fileManager.processFrontMatter`, which rewrites the frontmatter block and leaves the body byte-identical. There are two, each in one method:
  ...
  - **The user's own properties**, in `view.setProperty`, for the context menu's editing. Decided 2026-08-19, having previously been out of bounds; a first attempt at this from the detail view was reverted the same day, so the surface is the context menu and nothing else.
