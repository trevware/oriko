...
- **The plugin writes exactly two things, both through `app.fileManager.processFrontMatter`**, which rewrites the frontmatter block and leaves the body byte-identical. Each lives in one method and nowhere else:
  - **`grid`**, in `view.assign`, when a clipping is moved between grids.
  - **The user's own properties**, in `view.setProperty`, for the detail view's inline editing. Decided 2026-08-19, having previously been out of bounds.
- **`view.setProperty` refuses the Web Clipper's keys**, and the refusal is in the write method rather than in the UI that calls it, so a future caller cannot widen the licence by accident. Vault `CLAUDE.md` §9 calls `title`, `source`, `author`, `published`, `created`, `description` and `tags` a contract; `type`, `grid`, `cover` and `media` are the plugin's own. Everything else is the user's. The list is `editable.ts`, which is pure and tested.
- **Do not grow this into a general licence to edit notes.** Any *third* write is a new decision, not a precedent these two have already set.
